// Database setup script - run this to create tables and seed data
const { Pool } = require('pg');

const DATABASE_URL = process.env.DATABASE_URL || 'postgresql://postgres:MxiHqpAFeTEQzhXWqdMkabrJpRGmqQFL@caboose.proxy.rlwy.net:26525/railway';
...
const seedData = [
    ...
    },
    {
        ...
        arab: 'اللَّهُمَّ إِنِّي أَسْأَلُكَ خَيْرَ الْمَوْلَجِ وَخَيْرَ الْمَخْرَجِ بِاسْمِ اللهِ وَلَجْنَا، وَبِاسْمِ اللهِ خَرَجْنَا، وَعَلَى اللَّهِ رَبِّنَا تَوَكَّلْنَا',
        latin: "Allâhumma innî as'aluka khairal maulaji wa khairal makhraji, bismillâhi walajna wa bismillâhi kharajna wa 'ala-Llâhi rabbinâ tawakkalnâ",
        arti: 'Ya Allah, aku memohon kepada-Mu sebaik-baik tempat masuk dan sebaik-baik tempat keluar. Atas nama-Mu kami masuk dan atas nama-Mu kami keluar.',
    ...
    },
    {
        ...
        arti: 'Tuhanku, ampunilah diriku dan kedua orang tuaku, sayangilah mereka sebagaimana mereka menyayangiku di waktu aku kecil.',
    ...
    },
    {
        ...
        latin: "Ghufrânaka alḥamdulillâhil-ladzî adzhaba 'annil adzâ wa 'âfânî",
        arti: 'Dengan mengharap ampunan-Mu, segala puji bagi Allah yang telah menghilangkan penyakit dari tubuhku dan menyehatkan aku.',
        keywords: ['keluar kamar mandi', 'mandi', 'kamar mandi', 'sehat']
    },
    {
        label: 'DOA_SHOLAT_DHUHA',
        judul: 'Doa Sholat Dhuha',
        arab: 'اَللّهُمَّ إِنِّى أَسْأَلُكَ فَضْلَ الْـدُّحَى',
    ...
    },
    {
        ...
        arab: 'الْحَمْدُ لِلَّهِ الَّذِي أَحْيَانَا بَعْدَمَا أَمَاتَنَا',
        latin: "Alhamdulillahil-ladhi ahyana ba'dama amatana",
        arti: 'Segala puji bagi Allah yang telah menghidupkan kami setelah mematikan kami.',
        keywords: ['bangun tidur', 'doa bangun tidur']
    },
    {
        label: 'DOA_MINTA_KESEHATAN',
        judul: 'Doa Memohon Kesehatan',
        arab: 'اللَّهُمَّ عَافِنِي فِي بَدَنِي',
    ...
    },
    {
        ...
        arab: 'بَارَكَ اللهُ لَنَا فِيْمَا رَزَقَنَا',
    ...
    },
    {
        ...
        arab: 'أَسْتَغْفِرُ اللَّهَ رَبِّي',
    ...
    },
    {
        ...
        arab: 'اللَّهُمَّ أَنْتَ السَّلاَمُ وَمِنْكَ السَّلاَمُ',
    ...
    },
    {
        ...
        arab: 'اللَّهُمَّ اجْعَلْ خَيْرَ مَا أَسْمَعُ',
        latin: "Allahumma aj'al khaira ma asma'u",
        arti: 'Ya Allah, jadikanlah yang terbaik yang aku dengar (menjelang ajal).',
        keywords: ['mati', 'menjelang mati', 'doa akhir']
    },
    {
        label: 'DOA_MINTA_KEKUATAN',
        judul: 'Doa Meminta Kekuatan dan Kesabaran',
        arab: 'رَبِّ أَعُوذُ بِكَ مِنَ الْهَمِّ وَالْحَزَنِ',
        latin: "Rabb a'udzubika minal hammi wal hazan",
        arti: 'Ya Tuhanku, aku berlindung kepada-Mu dari kesedihan dan kesusahan.',
...
async function setup() {
    ...
    try {
        ...
        for (const doa of seedData) {
            await pool.query(
                `INSERT INTO doa (label, judul, arab, latin, arti, keywords) 
         VALUES ($1, $2, $3, $4, $5, $6)`,
                [doa.label, doa.judul, doa.arab, doa.latin, doa.arti, doa.keywords]
            );
        }
        console.log(`✅ Inserted ${seedData.length} doa`);

        // Verify
        const result = await pool.query('SELECT COUNT(*) FROM doa');
        console.log(`📊 Total doa in database: ${result.rows[0].count}`);
